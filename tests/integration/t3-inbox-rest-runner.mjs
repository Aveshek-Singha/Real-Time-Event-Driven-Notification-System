...
async function waitForInbox(port, recipientId, expectedCount) {
...
async function postJson(url, expectedStatus = 200) {
...
try {
  ...
  await postEvent(port, olderEvent);
  await postEvent(port, middleEvent);
  await postEvent(port, newerEvent);

  const inbox = await waitForInbox(port, recipientId, 3);
  ...
  const firstPage = await getJson(`http://127.0.0.1:${port}/recipients/${recipientId}/inbox?limit=2`);
  ...
  const secondPage = await getJson(`http://127.0.0.1:${port}/recipients/${recipientId}/inbox?limit=2&offset=2`);
  ...
  assert.deepEqual(
    await getJson(`http://127.0.0.1:${port}/recipients/${recipientId}/inbox/unread-count`),
  ...
  const afterRead = await getJson(`http://127.0.0.1:${port}/recipients/${recipientId}/inbox`);
  ...
  const unreadOnly = await getJson(`http://127.0.0.1:${port}/recipients/${recipientId}/inbox?unreadOnly=true`);
  ...
  assert.deepEqual(
    await postJson(`http://127.0.0.1:${port}/recipients/${recipientId}/inbox/read-all`),
  ...
  const emptyUnreadOnly = await getJson(`http://127.0.0.1:${port}/recipients/${recipientId}/inbox?unreadOnly=true`);
