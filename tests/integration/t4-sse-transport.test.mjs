...
describe("T4 SSE transport parity", () => {
  it("pushes the same Notification payload to mixed WS and SSE Connections", { timeout: 420_000 }, async () => {
    ...
    const runner = spawn(process.execPath, [
      ...
      "--import",
      "tsx",
      "tests/integration/t4-sse-transport-runner.mjs",
    ], {
      cwd: process.cwd(),
      env: {
        ...process.env,
        KAFKA_BROKERS: infrastructure.kafkaBroker,
        KAFKA_GROUP_ID: `t4-${Date.now()}`,
        KAFKA_TOPIC: `notifications-t4-${Date.now()}`,
        PORT: String(port),
